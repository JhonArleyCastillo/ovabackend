...
class ResilienceService:
    ...
    def resilient_hf_call(
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def wrapper(*args, **kwargs):
                ...
                # Configurar retry con Hyx
                @retry(
                    create_counter(retry_attempts),
                    expo(base=1, max_time=10),  # Backoff exponencial
    ...
    def simple_retry(attempts: int = 3, delay: float = 1.0):
        ...
        def decorator(func: Callable) -> Callable:
            ...
            async def wrapper(*args, **kwargs):
                @retry(
                    create_counter(attempts),
                    expo(base=delay, max_time=delay * 8),  # Backoff exponencial
                    (ConnectionError, TimeoutError, OSError, Exception)
                )
